...
block_decrypt(Cipher, Key, CipherText)
	...
	case block_cipher(Cipher) of
		{crypto, aes_ecb} ->
			<< << (crypto:crypto_one_time(aes_ecb, Key, Block,false))/binary >> || << Block:128/bitstring >> <= CipherText >>;
		{Module, BlockCipher} ->
...
block_encrypt(Cipher, Key, PlainText)
	...
	case block_cipher(Cipher) of
		{crypto, aes_ecb} ->
			<< << (crypto:crypto_one_time(aes_ecb, Key, Block,true))/binary >> || << Block:128/bitstring >> <= PlainText >>;
		{Module, BlockCipher} ->
